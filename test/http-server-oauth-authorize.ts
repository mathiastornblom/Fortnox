...
const port = 3333;
const scope =
    'article customer costcenter price invoice supplier supplierinvoice';
...
const authUrl = `https://apps.fortnox.se/oauth-v1/auth?${makeQueryString(
    fortnoxAuthReq
)}`;
...
const server = http.createServer((req, res) => {
    ...
    if (url.pathname === '/authorize') {
        ...
        authorize('https://apps.fortnox.se/oauth-v1/token', params, {
